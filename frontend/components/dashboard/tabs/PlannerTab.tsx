    ...
    <div>
      <div style={{background:sf,border:`1px solid ${br}`,borderRadius:12,padding:16,marginBottom:18,boxShadow:sh}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:10,marginBottom:10}}>
          <input value={newGoal.title} onChange={e=>setNewGoal(p=>({...p,title:e.target.value}))} placeholder="Goal title" style={inp}/>
          <select value={newGoal.period} onChange={e=>setNewGoal(p=>({...p,period:e.target.value as any}))} style={{...inp,appearance:'none' as any}}>{['yearly','quarterly','monthly'].map(p=><option key={p} value={p}>{p.charAt(0).toUpperCase()+p.slice(1)}</option>)}</select>
          <input type="number" value={newGoal.target} onChange={e=>setNewGoal(p=>({...p,target:+e.target.value}))} min={1} placeholder="Target" style={inp}/>
          <input value={newGoal.unit} onChange={e=>setNewGoal(p=>({...p,unit:e.target.value}))} placeholder="unit" style={inp}/>
        </div>
        <button onClick={()=>{ if(newGoal.title.trim()){ saveGoals([...goals,{id:Date.now().toString(),...newGoal,progress:0}]); setNewGoal({title:'',period:'monthly',target:10,unit:'tasks',color:'#00dcff'}); }}} style={{padding:'8px 20px',background:accent,border:'none',borderRadius:8,color:'#000',fontWeight:700,cursor:'pointer',fontFamily:'inherit',fontSize:'.84rem'}}>Add Goal</button>
      </div>
      {goals.length===0?<div style={{textAlign:'center',padding:'28px 0',color:t3}}><div style={{fontSize:'2rem',marginBottom:8}}>🎯</div><div>Add your first goal above</div></div>
            ...
            {gl.map(g=>{ const pct=Math.min(100,Math.round((g.progress/g.target)*100)); return <div key={g.id} style={{background:sf,borderTop:`1px solid ${br}`,borderRight:`1px solid ${br}`,borderBottom:`1px solid ${br}`,borderLeft:`3px solid ${g.color}`,borderRadius:10,padding:'13px 15px',marginBottom:8,boxShadow:sh,cursor:'pointer',transition:'.2s'}} onClick={(e)=>{ if((e.target as any).tagName!=='BUTTON') setSelectedCanvasGoal(g.id); }}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:7}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  <span style={{fontFamily:'Orbitron,monospace',fontWeight:700,fontSize:'.88rem',color:g.color}}>{g.progress}/{g.target} {g.unit}</span>
                  <button onClick={()=>saveGoals(goals.map(x=>x.id===g.id?{...x,progress:Math.min(x.target,x.progress+1)}:x))} style={{width:22,height:22,borderRadius:5,background:`${g.color}22`,border:`1px solid ${g.color}44`,color:g.color,cursor:'pointer',fontSize:'.85rem',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700}}>+</button>
                  <button onClick={()=>saveGoals(goals.filter(x=>x.id!==g.id))} style={{background:'none',border:'none',color:t3,cursor:'pointer'}}>✕</button>
                </div>
              </div>
              <div style={{height:7,background:isDark?'rgba(255,255,255,.07)':'rgba(100,130,200,.10)',borderRadius:4,overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:pct>=100?'#00c97a':g.color,borderRadius:4,transition:'.5s'}}/></div>
              <div style={{fontSize:'.68rem',color:t3,marginTop:3,textAlign:'right'}}>{pct}%</div>
            </div>; })}
          </div>;
        })}
    </div>
