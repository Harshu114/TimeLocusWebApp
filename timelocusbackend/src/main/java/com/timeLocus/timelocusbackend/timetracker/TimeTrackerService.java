...
public class TimeTrackerService {
    ...
    public List<TimeTrackerController.TimeEntryDTO> getEntriesForDate(User user, LocalDate date) {
        return repo.findByUserAndDateOrderByStartTimeAsc(user, date)
                .stream().map(this::toDTO).toList();
    }

    // ── Get entries in range ──────────────────────────────────────────────────
    public List<TimeTrackerController.TimeEntryDTO> getEntriesInRange(User user, LocalDate from, LocalDate to) {
        return repo.findByUserAndDateBetweenOrderByDateAscStartTimeAsc(user, from, to)
                .stream().map(this::toDTO).toList();
    }

    // ── Daily summary ─────────────────────────────────────────────────────────
    public TimeTrackerController.DailySummaryDTO getDailySummary(User user, LocalDate date) {
        ...
        List<TimeTrackerController.CategoryBreakdown> breakdown = byCategory.entrySet().stream()
                .map(e -> new TimeTrackerController.CategoryBreakdown(
                        e.getKey(), e.getValue(),
                        total > 0 ? (double) e.getValue() / total * 100 : 0
                )).toList();

        return new TimeTrackerController.DailySummaryDTO(
    ...
    // ── Weekly summary ────────────────────────────────────────────────────────
    public List<TimeTrackerController.DailySummaryDTO> getWeeklySummary(User user) {
        LocalDate monday = LocalDate.now().with(DayOfWeek.MONDAY);
        List<TimeTrackerController.DailySummaryDTO> result = new ArrayList<>();
    ...
    // ── Create manual entry ───────────────────────────────────────────────────
    @Transactional
    public TimeTrackerController.TimeEntryDTO createEntry(User user, TimeTrackerController.CreateEntryRequest req) {
    ...
    // ── Start live timer ──────────────────────────────────────────────────────
    @Transactional
    public TimeTrackerController.TimeEntryDTO startTimer(User user, String task, String category) {
        TimeEntry entry = TimeEntry.builder()
                .user(user).task(task).date(LocalDate.now())
    ...
    // ── Stop live timer ───────────────────────────────────────────────────────
    @Transactional
    public TimeTrackerController.TimeEntryDTO stopTimer(User user, String id) {
    ...
    // ── Update entry ──────────────────────────────────────────────────────────
    @Transactional
    public TimeTrackerController.TimeEntryDTO updateEntry(User user, String id,
                                                          TimeTrackerController.CreateEntryRequest req) {
        TimeEntry entry = repo.findByIdAndUser(id, user)
                .orElseThrow(() -> new RuntimeException("Entry not found"));
    ...
    // ── Entity → DTO ──────────────────────────────────────────────────────────
    private TimeTrackerController.TimeEntryDTO toDTO(TimeEntry e) {
        return new TimeTrackerController.TimeEntryDTO(
                e.getId(), e.getTask(),
